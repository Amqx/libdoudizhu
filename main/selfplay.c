...
static void printWeights(const BotWeights* weights) {
    const int count = botWeightsCount();
    for (int i = 0; i < count; i++) {
        printf("%s=%d\n", botWeightsName(i), botWeightsGet(weights, i));
    }
}
...
int main(const int argc, char** argv) {
    ...
    if (argc > 1)
        cfg.generations = atoi(argv[1]);
    if (argc > 2)
        cfg.games_per_generation = atoi(argv[2]);
    if (argc > 3)
        cfg.initial_step = atoi(argv[3]);
    if (argc > 4)
        cfg.seed = (unsigned int) strtoul(argv[4], NULL, 10);
    ...
    printf("final_arena completed=%d skipped=%d landlord=%d peasants=%d"
           ...
           result.final_arena.completed_games, result.final_arena.skipped_games, result.final_arena.landlord_wins,
           result.final_arena.peasant_wins, result.final_arena.seat_wins[0], result.final_arena.total_score);
    printWeights(&result.best_weights);
    return 0;
}
